...
extension UIImage {
    func convertOriginalImagetoBase64() -> String? {
    ...
    {
        let imageData = self.jpegData(compressionQuality: 0.1)
        return "data:image/jpeg;base64,"+(imageData?.base64EncodedString(options: Data.Base64EncodingOptions.lineLength64Characters))!
    }
    
    func convertCompressedImagetoBase64(compressionRate : CGFloat) -> String
    {
        let imageData = self.jpegData(compressionQuality: compressionRate)
    ...
    //        let image = self.compressImage(compressionRate: 0.1)
    //
    //        AddImageRequest.shared.image = image.convertUIImageToBase64()
    //        AddImageRequest.shared.userId = Global.shared.currentLoginUser.id
    //
    //
    //        serverRequest.delegate = self
    //        serverRequest.requestWithHeaderandBody(vc: self, url: Apis.addPicture, params: AddImageRequest.shared.returnAddImageParams(), method: .post, type: "addPicture", loading: true)
    //    }
    ...
    func downloadImage(imageUrl : String!) -> UIImage
    {
        ...
        let data = NSData(contentsOf: url ?? URL(string: "")!)
        if data != nil
        {
            return UIImage(data: data! as Data)!
        }
        else{
            return UIImage(named: "d-avatar") ?? UIImage()
        }
    ...
    class var backArrow:UIImage{
        return UIImage(named: "back_arrow_ic")!
    }
    
    class var blankImage:UIImage{
        return UIImage(named: "")!
    }
    
    class var Checked:UIImage{
        return UIImage(named: "Checked")!
    }
    
    class var Unchecked:UIImage{
        return UIImage(named: "Unchecked")!
    }
    
    class var sendStory:UIImage{
        return UIImage(named: "Send Story")!
    }
    
    class var threeDots:UIImage{
        return UIImage(named: "Three Dots")!
    }
    
    class var dislike:UIImage{
        return UIImage(named: "buttonDislikes")!
    }
}
